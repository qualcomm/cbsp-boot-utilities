...
def _replace_ph(
    ...
    if meta_item_index >= 0:
        try:
            ...
            if meta_item_index < len(items):
                ...
                print(f"[!] meta_item_index={meta_item_index} out of range; item_size not updated")
    ...
    for ph_i, old_h in side_effect_phs:
        ...
        if pos != -1:
            ...
            print(f"[!] PH#{ph_i} SHA-384 not found in ELF binary; hash table not updated")
    ...
    for seg_idx in seg_indices_with_dtbs:
        ...
        if old_ph0_hash is not None:
            ...
            if pos != -1:
                ...
                print(f"[i] ELF-header segment (PH#0) SHA-384 updated at file 0x{pos:x}")
            else:
                print("[!] ELF-header segment (PH#0) SHA-384 not found in hash segment (non-fatal)")
